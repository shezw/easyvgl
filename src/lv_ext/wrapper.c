...
{
    return esy_view_clean(esy_view_transp(esy_view(parent)));
}
...
{
    lv_obj_set_x( view, esy_left_of( toView , offset) );
}

void    esy_right_to( LvView view, LvView toView, i16 offset )
{
    lv_obj_set_x( view, esy_right_of( toView , offset) );
}

void    esy_top_to( LvView view, LvView toView, i16 offset )
{
    lv_obj_set_y( view, esy_top_of( toView , offset) );
}

void    esy_bottom_to( LvView view, LvView toView, i16 offset )
{
    lv_obj_set_y( view, esy_bottom_of( toView , offset) );
}
